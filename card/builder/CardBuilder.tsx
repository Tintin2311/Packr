...
const DEFAULT_RARITY: Rarity = "bronze" as Rarity;
...
export default function CardBuilder({
  ...
  onValidate?: (card: CardDraft) => void;
}) {
  ...
  const firstElement = useMemo<ElementKey>(
    () => elementKeys[0] ?? ("Feu" as ElementKey),
    [elementKeys]
  );

  const [element, setElement] = useState<ElementKey>(firstElement);
  const rarity = DEFAULT_RARITY; // verrouillé (pas affiché)
  ...
  const handleValidate = useCallback(async () => {
    ...
    if (name.length < 2) {
      Alert.alert("Nom affiché", "Indique au moins 2 caractères.");
      return;
    }
    if (!/^\d{2}\/\d{2}\/\d{4}$/.test(birthdateInput) || age == null) {
      Alert.alert("Date de naissance", "Entre une date valide au format JJ/MM/AAAA.");
      return;
    }
    ...
    try {
      ...
      const userId = auth.user?.id;
      if (!userId) {
        Alert.alert("Authentification", "Tu dois être connecté.");
        return;
      }
        ...
        .upsert(
          {
            ...
            rarity: payload.rarity, // bronze en interne
    ...
    <SafeAreaView style={[styles.screen, { paddingTop: insets.top || 12 }]}>
      ...
      >
        ...
        <View style={styles.form}>
          ...
          <View style={styles.choicesRow}>
            {elementKeys.map((k) => (
              <Pressable
                ...
                style={[styles.choice, element === k && styles.choiceActive]}
              >
          ...
          {/* CTA Valider */}
          <Pressable
            onPress={saving ? undefined : handleValidate}
